...
func BenchmarkChiRouter(b *testing.B) {
	...
	router := NewRouter()
	router.Handle(http.MethodGet, "/api/{articleSlug:[a-z-]+}", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	}))
	w := &mockedResponseWriter{}
	r, _ := http.NewRequest(http.MethodGet, "/api/home-is-toronto", nil)
	for i := 0; i < b.N; i++ {
		router.ServeHTTP(w, r)
	}
}
